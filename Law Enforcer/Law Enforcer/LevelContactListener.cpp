...
void LevelContactListener::BeginContact(b2Contact *contact) {
    ...
    else if (fPlayerHurtSensor != nullptr)
    {
		...
		else if (other != nullptr && (boss->get_is_dashing() && other->GetUserData() == (void*)boss_dash_hitbox_right) && boss->get_is_facing_right()) {
		...
		else if (other != nullptr && (boss->get_is_dashing() && other->GetUserData() == (void*)boss_dash_hitbox_left) && !boss->get_is_facing_right()) {
